<template>
  ...
  <div id="errorSearch" class="fs-xs" uk-modal>
    <div class="uk-modal-dialog">
        <div class="uk-modal-header bg-transparent">
          <div class="header-search-form uk-position-relative">
            ...
            <input v-model="search" ref="errorSearchField" type="text" class="bg-light fs-m uk-input uk-form-blank uk-input-large" placeholder="Search hotels, destinations or guides..."/>
        ...
        <div class="header-search-results uk-modal-body">
          <ul v-if="filteredList(locations).length" class="search-results-list search-results-destinations uk-list">
            ...
            <li v-for="(location, index) in filteredList(locations)" :key="index" class="uk-position-relative">
              <NuxtLink :to="navigateToDestination(location.id, location.slug, location.parent)" class="color-dark fw-medium use-transition" uk-toggle="target: #errorSearch" :prefetch="false">{{location.title}}</NuxtLink>
            </li>
          </ul>
          ...
          <ul v-if="filteredList(guides).length" class="search-results-list search-results-guides uk-list">
            ...
            <li v-for="(guide, index) in filteredList(guides)" :key="index" class="uk-position-relative">
              <NuxtLink :to="'/guides/'+guide.slug" class="color-dark fw-medium use-transition" uk-toggle="target: #errorSearch" :prefetch="false">{{guide.title}}</NuxtLink>
            </li>
          </ul>
          ...
          <ul v-if="filteredList(hotels).length" class="search-results-list search-results-hotels uk-list">
            ...
            <li v-for="(hotel, index) in filteredList(hotels)" :key="index" class="uk-position-relative">
              <NuxtLink :to="'/hotels/'+hotel.slug" class="color-dark fw-medium use-transition" uk-toggle="target: #errorSearch" :prefetch="false">{{hotel.title}}</NuxtLink>
            </li>
          </ul>
        </div>
    </div>
...
<script setup>
  ...
  onMounted(() => {
    UIkit.util.on('#errorSearch', 'shown', () => {
      setTimeout(() => {
        errorSearchField.value.focus()
      }, 100)
    });
  })
...
<style>
  #errorSearch .uk-modal-dialog {
    border-radius: 20px;
  }
  #errorSearch .uk-modal-header {
